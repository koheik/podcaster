...
class Entry < ActiveRecord::Base
  ...
  if (ENV['HOST'] =~ /sakura\.ne\.jp/)
    @@dbdir = '/home/koheik/db/'
    @@xmldir = '/home/koheik/www/podcast/'
    @@cachedir = '/home/koheik/www/podcast/files/'
  end
  @@dbfile = @@dbdir + 'podcast.sqlite'
  def self.dbfile
    @@dbfile
  end
end
...
class Utamaru < Entry
  @maxid = 0
  @@xmlfile = @@xmldir + 'hustler.xml'

  PODCAST_KEYWORDS = ["ライムスター宇多丸",
  ...
  def self.parse_and_add(urlstr)
    ...
    doc.xpath("//div[@class='entry']").each do |ent|
       ...
       nuent = Entry.new(:eid => eid, :dtime => edt, :category => 1,
                         :link => elk, :head => ehd, :body => ebd);
       nu = true;
       Entry.where(:eid => eid, :category => 1).each do |old|
         nu &= !nuent.wequal(old)
       end
       nuent.save if nu
     end
   ...
   def self.update_db
     ...
     (2..@maxid).each do |i|
       urlstr = "http://www.tbsradio.jp/utamaru/podcast/index_#{i}.html"
   ...
   def self.update_index
    File.open(@@xmlfile, "w+") { |f|
      print_podcast_header(f)
      find_hustler.each do |ent|
        ent.print_podcast_item(f)
      end
      print_podcast_footer(f)
    }
  end
  ...
  def self.find_hustler()
     keywords = ["hustler",
                 ...
                 "ENDMARK"]
     ary = []
     all(:conditions => ["category = ?", 1], :order => "eid desc").each do |entry|
       idx = -1
...
class Kirakira < Entry
  @@maxid = 400
  @@xmlfile = @@xmldir + 'machiyama.xml'
  
  PODCAST_KEYWORDS = ["キラキラ", "キラ☆キラ", "小島慶子",
      "ラジオ", "TBS", "赤坂ブロガー祭り",
   ...
   def self.parse_and_add(urlstr)
    ...
    doc.xpath("//div[@class='entry']").each do |ent|
      # fetch entry id
      eid = ent.xpath("./a").first.attr('id').gsub(/^a/, '').to_i
      ehd = Entry.normalize(ent.xpath("h2").first.to_s)
      ebd = Entry.normalize(ent.xpath(".//div[@class='entry-body']").first.to_s)

      # fetch date
      dtime = ent.xpath("./p[@class='posted']").first.to_s
      dtime.gsub!("\346\227\245\346\231\202", "日時")
      dtime.gsub!("\345\271\264", "年")
      dtime.gsub!("\346\234\210", "月")
      dtime.gsub!("\346\227\245", "日")

      if (dtime =~ /(\d+)年\s*(\d+)月\s*(\d+)日（.+）(\d+)時(\d+)分更新/)
        edt = DateTime.civil($1.to_i, $2.to_i, $3.to_i, $4.to_i, 
                                $5.to_i, 0, Rational(9, 24))
      else
          puts "Error: url=#{urlstr}, eid=#{eid}, dtime=#{dtime}"
      ...
      # fetch link
      elk = ent.xpath("./p[@class='posted']/a[@class='permalink']").first.attr('href')
 
      nuent = Entry.new(:eid => eid, :dtime => edt, :category => 2,
                         :link => elk, :head => ehd, :body => ebd);
      nu = true;
      Entry.where(:eid => eid, :category => 2).each do |old|
        nu &= !nuent.wequal(old)
      end
      nuent.save if nu
    end
  ...
  def title
    hd = Nokogiri::HTML.parse(head).xpath("//h2").text
  end
  
  def body_clean
    bdy = Nokogiri::HTML.parse(body).xpath("//div[@class='entry-body-text']").text
    CGI::escapeHTML(bdy)
  end
  
  def self.update_db
    if (true)
      urlstr = "http://www.tbsradio.jp/kirakira/pod/index.html"
      parse_and_add(urlstr)
    end

    (2..@@maxid).each do |i|
      urlstr = "http://www.tbsradio.jp/kirakira/pod/index_#{i}.html"
      break unless parse_and_add(urlstr)
    end
  end
  ...
  def self.find_machiyama
    keywords = ["町山", "ENDMARK"]
    ary = []
    all(:conditions => ['category = ?', 2], :order => "eid desc").each do |entry|
      idx = -1
      (0...keywords.length).each do |i|
        if (entry.body.to_s.include?(keywords[i])) 
          idx = i
    ...
    ary
  end # self.find_machiyama
  
  def self.download
    find_machiyama.each do |entry|
      locs = entry.locals
      unless(locs.size > 0)
        dst = File.basename(entry.file_name)
        dst = @@cachedir + dst
        open(dst, 'wb') do |f|
          f << open(entry.file_name).read
        end
        Local.create(:entry => entry,
                      :fpath => dst,
                      :fsize => File.size(dst),
                      :finode => File.stat(dst).ino)
      end
    end
  end
  ...
  def self.print_podcast_header(f)
    ...
    f.print "<channel>\n"
    f.print "<ttl>60</ttl>\n"
    f.print "<title>小島慶子　キラ☆キラ ― 町山編 －</title>\n"
    f.print "<link>http://www.tbsradio.jp/kirakira/</link>\n"
    f.print "<description>TBSラジオで月曜～金曜お昼1時から放送！「くだらない」「どうでもいい」一見そんな風に見えるメッセージテーマを日替わりでお出ししております。でも、あ～ら不思議！くだらなく見えたことを、みんなでああだこうだ言ってるうちに、意外な発見が立体視のように浮かび上がって来ます！午後のひととき、そんなキラキラしたラジオ体験を「小島慶子キラ☆キラ」でしてみませんか？メール：kirakira@tbs.co.jp</description>\n"
    f.print "<language>ja</language>\n"
    f.print "<lastBuildDate>#{last_build}</lastBuildDate>\n"
    f.print "<copyright>Tokyo Broadcasting System, Inc. All Rights Reserved.</copyright>\n"
    f.print "<category>Comedy</category>\n"
    f.print "<itunes:category text=\"Comedy\"></itunes:category>\n"
    f.print "<itunes:subtitle>毎週月～金 13:00～15:30 放送中！</itunes:subtitle>\n"
    f.print "<image>\n"
    f.print "  <url>http://www.tbsradio.jp/kirakira/300_300.jp</url>\n"
    f.print "  <link>http://www.tbsradio.jp/kirakira/</link>\n"
    f.print "  <title>小島慶子　キラ☆キラ</title>\n"
    f.print "</image>\n"
    f.print "<itunes:author>TBS RADIO 954kHz</itunes:author>\n"
    f.print "<itunes:summary>TBSラジオで月曜～金曜お昼1時から放送！「くだらない」「どうでもいい」一見そんな風に見えるメッセージテーマを日替わりでお出ししております。でも、あ～ら不思議！くだらなく見えたことを、みんなでああだこうだ言ってるうちに、意外な発見が立体視のように浮かび上がって来ます！午後のひととき、そんなキラキラしたラジオ体験を「小島慶子キラ☆キラ」でしてみませんか？メール：kirakira@tbs.co.jp</itunes:summary>\n"
    f.print "<generator>http://www.sixapart.com/movabletype/</generator>\n"
    f.print "<itunes:owner>\n"
    f.print "  <itunes:name>TBS RADIO 954kHz</itunes:name>\n"
    f.print "  <itunes:email>kirakira@tbs.co.jp</itunes:email>\n"
    f.print "</itunes:owner>\n"
    f.print "<itunes:image href=\"http://www.tbsradio.jp/kirakira/300_300.jpg\" />\n"
  end
  
  def print_podcast_item(f)
    itm_date = dtime.strftime("%a, %d %b %Y %X %z")
#    itm_titl = dtime.strftime("%Y/%m/%d") + " " + title_clean
    itm_titl = title_clean
    if (locals.size > 0)
      fn = locals.first.fpath
      lnk = fn.gsub("/home/koheik/www", "http://koheik.sakura.ne.jp")
    else
      lnk = file_name
    end
    ...
    f.print "  <itunes:keywords>" + PODCAST_KEYWORDS.join(", ") + "</itunes:keywords>\n"
    f.print "  <itunes:explicit>no</itunes:explicit>\n"
    f.print "  <enclosure url=\"#{lnk}\" type=\"audio/mpeg\" />\n"
    f.print "</item>\n" 
  end
end
...
class Tsutaya < Entry
  @@maxid = 400
  @@xmlfile = @@xmldir + 'tsutaya.xml'
  
  PODCAST_KEYWORDS = ["伊集院光", "伊集院", "TSUTAYA", "ツタヤ", "週末", "たまむすび", "映画", "DVD", "ラジオ", "TBS", "TBSラジオ"]
 
  def title
    
  end
 
  def self.parse_and_add
    dnames = ["/home/koheik/www/podcast/files"]
    dnames.each do |dname|
      Dir.glob("#{dname}/*.mp3").each do |fn|
        if (fn =~ /Tsutaya_(\d+)_(\d+)_(\d+).mp3/)
          y = $1.to_i
          m = $2.to_i
          d = $3.to_i
          t = 100*(100*y + m) + d
          ...
          eid = sprintf("%4d%02d%02d", y, m, d)
          edt = DateTime.civil(y.to_i, m.to_i, d.to_i, 0, 0, 0, Rational(9, 24))
          ehd = ""
          ebd = ""
          nuent = Entry.new(:eid => eid, :dtime => edt, :category => 3,
                         :link => fn, :head => ehd, :body => ebd);
          nu = true;
          Entry.where(:eid => eid, :category => 3).each do |old|
            nu &= !nuent.wequal(old)
          end
          nuent.save if nu
        end
  ...
  def self.find_tsutaya
    keywords = ["町山", "ENDMARK"]
    ary = []
    all(:conditions => ['category = ?', 3], :order => "eid desc").each do |entry|
      ary << entry
    end
    ary
  end # self.find_tsutaya
  ...
  def self.print_podcast_header(f)
    ...
    f.print "<channel>\n"
    f.print "<ttl>60</ttl>\n"
    f.print "<title>伊集院光の週末TSUTAYAに行ってこれ借りよう</title>\n"
    f.print "<link>http://www.tbs.co.jp/radio/tsutaya/</link>\n"
    f.print "<description>DVDを借りて映画でも観ようかなぁ・・・。でも何を見たらいいかわからないなぁ・・・という人に向けて映画好きの著名人がおすすめの映画を紹介！その映画を伊集院光も観て、すすめた本人にその感想を2週間後に報告！映画を観た人は、伊集院光と感想を照らし合わせることが出来るという映画支援番組。TBSラジオ「たまむすび」内で毎週金曜15時00分頃～放送！</description>\n"
    f.print "<language>ja</language>\n"
    f.print "<lastBuildDate>#{last_build}</lastBuildDate>\n"
    f.print "<copyright>Tokyo Broadcasting System, Inc. All Rights Reserved.</copyright>\n"
    f.print "<category>Comedy</category>\n"
    f.print "<itunes:category text=\"Comedy\"></itunes:category>\n"
    f.print "<itunes:subtitle>TBSラジオ「たまむすび」内 毎週金曜日 午後15時00分頃～放送中！</itunes:subtitle>\n"
    f.print "<image>\n"
    f.print "  <url>http://www.tbsradio.jp/tsutaya/300_300.jpg</url>\n"
    f.print "  <link>http://www.tbs.co.jp/radio/tsutaya/</link>\n"
    f.print "  <title>伊集院光の週末TSUTAYAに行ってこれ借りよう</title>\n"
    f.print "</image>\n"
    f.print "<itunes:author>TBS RADIO 954kHz</itunes:author>\n"
    f.print "<itunes:summary>DVDを借りて映画でも観ようかなぁ・・・。でも何を見たらいいかわからないなぁ・・・という人に向けて映画好きの著名人がおすすめの映画を紹介！その映画を伊集院光も観て、すすめた本人にその感想を2週間後に報告！映画を観た人は、伊集院光と感想を照らし合わせることが出来るという映画支援番組。TBSラジオ「たまむすび」内で毎週金曜15時00分頃～放送！</itunes:summary>\n"
    f.print "<generator>http://www.sixapart.com/movabletype/</generator>\n"
    f.print "<itunes:owner>\n"
    f.print "  <itunes:name>TBS RADIO 954kHz</itunes:name>\n"
    f.print "  <itunes:email>tsutaya@tbs.co.jp</itunes:email>\n"
    f.print "</itunes:owner>\n"
    f.print "<itunes:image href=\"http://www.tbsradio.jp/tsutaya/300_300.jpg\" />\n"
  end
  
  def print_podcast_item(f)
    itm_date = dtime.strftime("%a, %d %b %Y %X %z")
    itm_titl = "TSUTAYA " + eid.to_s
    itm_lnk = link.gsub("/home/koheik/www", "http://koheik.sakura.ne.jp")
    body_clean = ""
    f.print "<item>\n"
    f.print "  <title>#{itm_titl}</title>\n"
    f.print "  <link>#{link}</link>\n"
    ...
    f.print "  <itunes:keywords>" + PODCAST_KEYWORDS.join(", ") + "</itunes:keywords>\n"
    f.print "  <itunes:explicit>no</itunes:explicit>\n"
    f.print "  <enclosure url=\"#{itm_lnk}\" type=\"audio/mpeg\" />\n"
    f.print "</item>\n" 
  end
end
...
if __FILE__ == $0
  if (true)
    Utamaru.update_db
    Utamaru.update_index
  end

  if (false)
    Kirakira.update_db
    Kirakira.download
    Kirakira.update_index
  end
  
  if (true)
    Tsutaya.update_db
    Tsutaya.update_index
  end
end
# end of file
